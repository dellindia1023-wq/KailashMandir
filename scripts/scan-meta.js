...
async function main() {
  ...
  if (await exists(path.join(process.cwd(), 'index.html'))) targets.push(process.cwd());
  ...
  for (const r of results) {
    ...
    if (r.canonical && r.canonical === '') notes.push('missing-canonical');
  ...
  await fs.writeFile('meta_report.csv', csvLines.join('\n'), 'utf8');
  ...
  await fs.writeFile('meta_report.md', md.join('\n'), 'utf8');
