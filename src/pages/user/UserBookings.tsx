                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  {booking.payment_status === "pending" && (
                    <div className="mt-4 rounded-2xl border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-900">
                      ...
                      <p className="mt-1 text-xs text-yellow-900/80">Your booking has been reserved, but payment has not completed yet. If you’ve already paid, please wait a few minutes for confirmation.</p>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => window.location.href = "mailto:kailashmahadevagra@gmail.com"}
                          className="text-xs"
                        >
                          Contact Support
                        </Button>
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const { data: orderData, error } = await supabase.functions.invoke("create-razorpay-order", { body: { bookingId: booking.id } });
                              if (error) throw error;
                              await openRazorpayCheckout({ orderId: orderData.orderId, bookingId: orderData.bookingId, amount: orderData.amount, currency: orderData.currency, keyId: orderData.keyId }, `${booking.pujas.name} Booking`, user?.email || "");
