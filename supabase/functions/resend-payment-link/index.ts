...
serve(async (req: Request) => {
  ...
  try {
    ...
    let body: unknown;
    try { body = await req.json(); } catch { return new Response(JSON.stringify({ error: "Invalid request body" }), { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders }); }
    ...
    const { data: booking, error: bookingErr } = await adminClient.from("puja_bookings").select("id, user_id, amount, puja_id, pujas(name)").eq("id", bookingId).maybeSingle();
    ...
    // Fetch user email
    const { data: userRecord } = await adminClient.auth.admin.getUserById(booking.user_id);
    ...
    try {
      const subject = `Complete payment for ${booking.pujas?.name || "your booking"}`;
      const react = PaymentLinkEmail({ recipientName: "Devotee", pujaName: booking.pujas?.name || "Puja", amount: Number(booking.amount || 0), payUrl });
