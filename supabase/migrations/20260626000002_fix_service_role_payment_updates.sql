-- Allow the Supabase service role to update payment fields during payment verification
-- while still blocking regular users from tampering with payment data.
...
BEGIN
  ...
  -- Allow backend/service-role updates (used by Edge Functions) and admins.
  IF current_role = 'service_role' OR has_role(auth.uid(), 'admin') THEN
    RETURN NEW;
  END IF;
...
BEGIN
  ...
  -- Prevent non-admins from changing donation payment fields.
